...
public class ReturnmoneyThread implements Runnable{
	...
	private ReturnmoneyService service;
	public ReturnmoneyThread() {
		service = new ReturnmoneyService();
		request = (HttpServletRequest)asyncContext.getRequest();
		service.setRequest(request);
	}
	public ReturnmoneyThread(AsyncContext asyncContext) {
		service = new ReturnmoneyService();
		this.asyncContext = asyncContext;
		request = (HttpServletRequest)asyncContext.getRequest();
		service.setRequest(request);
	}
	public ReturnmoneyThread(AsyncContext asyncContext,HttpSession session) {
		service = new ReturnmoneyService();
		this.asyncContext = asyncContext;
