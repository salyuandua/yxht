...
public class totalSalesThread implements Runnable{
	// 异步操作的上下文对象,通过构造方法传进来
		...
		public totalSalesThread() {
			service = new totalSalesService();
			request = (HttpServletRequest)asyncContext.getRequest();
			service.setRequest(request);
		}
		public totalSalesThread(AsyncContext asyncContext) {
			service = new totalSalesService();
			this.asyncContext = asyncContext;
			request = (HttpServletRequest)asyncContext.getRequest();
			service.setRequest(request);
		}
		public totalSalesThread(AsyncContext asyncContext,HttpSession session) {
			service = new totalSalesService();
			this.asyncContext = asyncContext;
