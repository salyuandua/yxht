...
public class EmailsHandleThread implements Runnable{
	...
	public EmailsHandleThread() {
		service = new EmailsService();
		request = (HttpServletRequest)asyncContext.getRequest();
		service.setRequest(request);
	}
	public EmailsHandleThread(AsyncContext asyncContext) {
		service = new EmailsService();
		this.asyncContext = asyncContext;
		request = (HttpServletRequest)asyncContext.getRequest();
		service.setRequest(request);
	}
	public EmailsHandleThread(AsyncContext asyncContext,HttpSession session) {
		service = new EmailsService();
		this.asyncContext = asyncContext;
