...
public class ProductHandleThread  implements Runnable{
	...
	public ProductHandleThread() {
		service = new ProductService();
		request = (HttpServletRequest)asyncContext.getRequest();
		service.setRequest(request);
	}
	public ProductHandleThread(AsyncContext asyncContext) {
		service = new ProductService();
		this.asyncContext = asyncContext;
		request = (HttpServletRequest)asyncContext.getRequest();
		service.setRequest(request);
	}
	public ProductHandleThread(AsyncContext asyncContext,HttpSession session) {
		service = new ProductService();
		this.asyncContext = asyncContext;
		...
		try {
			...
			try {
				...
				if(method==null){
					...
					out.print(service.getReturnString("action 错了，"));
				}else{
