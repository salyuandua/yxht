...
public class ProductPolicyHandleThread  implements Runnable{
	...
	private ProductPolicyService service;
	public ProductPolicyHandleThread() {
		service = new ProductPolicyService();
		request = (HttpServletRequest)asyncContext.getRequest();
		service.setRequest(request);
	}
	public ProductPolicyHandleThread(AsyncContext asyncContext) {
		service = new ProductPolicyService();
		this.asyncContext = asyncContext;
		request = (HttpServletRequest)asyncContext.getRequest();
		service.setRequest(request);
	}
	public ProductPolicyHandleThread(AsyncContext asyncContext,HttpSession session) {
		service = new ProductPolicyService();
		this.asyncContext = asyncContext;
