...
public class StaffRecordThread implements Runnable{
	...
	private StaffRecordService service;
	public StaffRecordThread() {
		service = new StaffRecordService();
		request = (HttpServletRequest)asyncContext.getRequest();
		service.setRequest(request);
	}
	public StaffRecordThread(AsyncContext asyncContext) {
		service = new StaffRecordService();
		this.asyncContext = asyncContext;
		request = (HttpServletRequest)asyncContext.getRequest();
		service.setRequest(request);
	}
	public StaffRecordThread(AsyncContext asyncContext,HttpSession session) {
		service = new StaffRecordService();
		this.asyncContext = asyncContext;
